...
const FlowDemo: React.FC<FlowProps> = ({ scenes, story, callback, choices }) => {
    ...
    useEffect(() => {
        ...
        const sceneEdges = scenes.flatMap((scene) =>
            scene.choices.map((choice) => ({
                id: choice.scene_slug + choice.next_scene_slug,
    ...
    const onConnect = useCallback(
        async (connection: Connection) => {
            ...
            var scn = scenes.find(s => s.slug == request.scene_slug);
            if (scn) {
                var newChoices = scn?.choices;
                newChoices?.push({ ...request } as unknown as Choice)
                localStorage.setItem(scn?.slug, JSON.stringify(newChoices))
    ...
    const onRemoveNode = useCallback(
        async (nodeId: string) => {
            ...
            var newScenes = scenes.filter(s => s.slug != nodeId);
    ...
    const onNodeDragStop = useCallback(async (event: React.MouseEvent, node: Node) => {
        ...
        if (scene) {
            ...
            var newScenes = scenes.filter(s => s.slug != scene.slug);
            var findS = scenes.find(s => s.slug == scene.slug);
            newScenes.push({ ...findS, x: request.x, y: request.y } as Scene)
            localStorage.setItem("scenes", JSON.stringify(newScenes))
        }
    }, [scenes, story.slug]);
    ...
    const handleDrop = async (event: React.DragEvent<HTMLDivElement>) => {
        ...
        const newScenes = [...scenes, { ...request, slug: newNode.id } as Scene];
        localStorage.setItem("scenes", JSON.stringify(newScenes));
        callback?.();
    };
